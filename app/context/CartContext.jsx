...
export function CartProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (items.length === 0 && !localStorage.getItem('talanquera_cart')) return;
 ...
 const decrease = async (lineId) => {
  ...
  const contextValue = useMemo(() => ({
    ...
    items, tipoOrden, total, metodoPago, propina, montoManual,
    // Las funciones no cambian porque no dependen de estados directos en su definición
    // pero las incluimos por seguridad de tipos
  ]);
